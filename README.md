...
`composer phpunit` covers the unit suite; the integration suite needs a real RabbitMQ with the
delayed-message plugin, which `bin/testbed` boots for you (same image CI uses, on ports that will
not collide with a broker you already run).
...
Needs docker or podman - whichever responds is picked, or set `CONTAINER_RUNTIME`. To use your
own broker instead, set `AMQP_TEST_HOST`, `AMQP_TEST_PORT`, `AMQP_TEST_HTTP_PORT`,
`AMQP_TEST_USER`, `AMQP_TEST_PASSWORD`, `AMQP_TEST_VHOST` and run `composer phpunit-integration`.
